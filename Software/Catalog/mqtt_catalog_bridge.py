...
# Trova il percorso della cartella principale 'Software' (un livello sopra 'Catalog')
...
class MQTTCatalogBridge:
    ...
    def on_message(self, client, userdata, msg):
        try:
            # 1. Decodifichiamo il payload JSON ricevuto dal dispositivo
            payload = json.loads(msg.payload.decode("utf-8"))
            print(f"\n[MQTT] Ricevuto messaggio su {msg.topic}: {payload}")
            
            # Verifichiamo se è un device o un service (il payload deve dirlo, es. "category": "devices")
            category = payload.get("category", "devices") # default a devices se non specificato
            ...
            # 2. STRUTTURA DEL REFRESH O INSERIMENTO (Usando il LOCK del vostro catalogo)
            with self.catalog_service.lock:
                # Se il dispositivo esiste già, facciamo solo il REFRESH del timestamp
                if item_id in self.catalog_service.catalog[category]:
                    ...
                    # Se è nuovo, inseriamo tutto il pacchetto aggiungendo il timestamp iniziale
                ...
                # Sfruttiamo la vostra funzione interna per salvare su file catalog.json
                self.catalog_service._save_catalog_unsafe()

            # 3. PUBBLICAZIONE DELL'ACKNOWLEDGEMENT (ACK)
            # Inviamo una conferma al topic specifico del dispositivo (richiesto dall'esercizio)
            response_topic = f"polito/iot/{category}/{item_id}/ack"
            ack_message = {
                "status": "success",
    ...
    def start(self):
        # Il bridge legge l'IP e la porta direttamente dalla memoria del catalogo
        broker_ip = self.catalog_service.catalog["broker"]["ip"]
        broker_port = self.catalog_service.catalog["broker"]["port"]

        print(f"[MQTT] Tentativo di connessione a {broker_ip}:{broker_port}...")
