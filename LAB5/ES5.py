...
class Catalog(object):
    ...
    def __init__(self):
        ...
        #struttura base del catalogo che contiene le indo del brocker al livello root
        self.catalog = {
                ...
                with self.lock:
                    ...
                    for category in ['devices','services']:
                        ...
                        for item_id in to_delete:
                            ...
                            cleaned = True
                if cleaned:
                    print(f"[{tiem.strftime('%X')}] CLEANUP Remuved record from catalog")
                    self._save_catalog_unsafe()
        ...
        def POST(self, *path, **params):
            #Crea o aggiorna(Upsert) un dispositivo o un servizio
            if not path or path[0] not in ['devices','service']:
        ...
        if __name__ == '__main__':
            conf = {
                '/':{
                    'requst.dispatch': cherrypy.dispatch.MethodDispatcher(),
